            ...
            <div className={`${poppins.className} min-h-screen bg-box`}>
                ...
                <div className="px-4">
                    <div className="glass-primary my-12 p-4 rounded border mx-auto max-w-7xl">
                        ...
                        <p className="text-lg text-gray-600">Total {words.length} words found</p>
                        <Searchbar />
                    </div>
                    ...
                    <div className="pt-8">
                        {loading ? (
                            ...
                            <div className="grid gap-4 lg:grid-cols-3 md:grid-cols-2 text-center">
                                {words.map((word, i) => (
                                    <div key={word._id} className="glass-primary p-4 rounded-lg shadow-md flex justify-between items-center">
                                        <span className="font-semibold text-lg">{word.word}</span>
                                        <Link href={`/dictionary/word/${word.word}`}>
                                            <button className="px-3 py-1 bg-blue-500 text-white rounded shadow hover:bg-blue-600">View</button>
                                        </Link>
                                    </div>
                                ))}
                            </div>
